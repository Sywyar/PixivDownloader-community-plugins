...
申请 PR 未合并关闭时，**Community candidate cleanup** 自动删除仍归属该申请的候选草稿及附件。它与归档、完成审核和正式发布共用队列，只执行受保护主线代码，通过仓库内置 token 清理，不依赖投稿者权限或向导选择。删除前重新读取 PR、Release 与资产；请求已重新打开、head 已变化、草稿已被新申请复用或 Release 已公开时保留。清理结果记录在 Actions 摘要中；读取、校验或权限错误使作业失败，不能视为已删除。草稿删除后重新投稿会重新建立候选；清理前已被新申请复用的草稿继续保留。该流程不追溯关闭事件发生前的历史残留，也不删除源码仓库 Release、已发布版本或 `operation/` 审核结果归档。

自审、Gate 和完成审核预检通过本次运行的 `GITHUB_TOKEN` 读取草稿。GitHub 的 Draft 可见性要求这些受保护作业具有 `contents: write`；它们固定执行主线代码，不执行投稿源码，也不需要投稿者提供令牌。权限不足报告 `CANDIDATE_ARCHIVE_READ_FORBIDDEN`，只有候选或证明尚未归档才显示等待。
