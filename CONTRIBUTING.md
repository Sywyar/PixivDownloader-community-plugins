...
自动合并与审核提交的标题均标明请求对象：版本操作显示 `发布者 / 插件-v版本`，密钥轮换显示 `发布者 publisherId`。发布结果与通知分别核验；主线正常前进后，通知仍校验原运行、源码祖先关系及受保护代码未变，并重新核对 PR head 和状态。空通知不会启动作业，签名和状态写入仍要求执行来源与当前主线一致。
