...
当前准入只接受仓库所有者从本仓库分支发起的维护 PR。数据 PR 的独立静态检查不授予准入权；源码审阅、隔离重建与发布执行器尚未接通，数据 PR 仍保持阻断。普通投稿者的权限边界是 fork PR，不授予本仓库 Write、Maintain 或 Admin 权限。投稿与工具维护须分开提交，不能在同一 PR 中修改投稿数据及其检查器。
