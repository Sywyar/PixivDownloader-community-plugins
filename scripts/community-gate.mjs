...
import { prepareSdk, evaluate } from './sdk.mjs';
import { gatePath, execution, facts, fingerprint, event, pull } from './platform.mjs';
...
export function publish(number, context, prepared, call = api, write = api, readGit) {
    ...
    try {
        ...
        const collect = () => attachDecisions(facts(number, prepared, context.current, call),
            loadDecisions(number, prepared, context.current, call, readGit));
        ...
        const summary = 'Operation: maintenance\n\nInput: ' + result.snapshot.inputSha256
        ...
        const labels = [...result.labels, 'type:maintenance'];
        // 维护合并没有目录应用动作；只有发布执行器能显示 awaiting-apply/completed。
        if (result.snapshot.state === 'MERGED') labels.splice(labels.indexOf('state:awaiting-apply'), 1);
        return { number, head, labels };
    } catch (error) {
        const failures = [];
        ...
        return { number, head, labels: ['ci:blocked', 'review:pending'], error: error.message };
    }
}
...
export function notify(projections, call = api) {
    for (const projection of projections) {
        ...
        if (pull(number, call).head.sha !== projection.head) continue;
...
export function gate() {
    ...
    let prepared;
    try { prepared = prepareSdk(); } catch (error) { prepared = error; }
    const projections = numbers.map(number => publish(number, context, prepared));
...
main(import.meta.url, () => {
    ...
    } else if (process.argv.length === 2) gate();
