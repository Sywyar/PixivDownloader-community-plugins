...
import { appendReviewCommit, checkResult, makeReceipt, receiptPath, receiptExpired, reviewPrerequisite, forkApi } from '../apply-result.mjs';
...
test('完成审核只快进原 PR，真实 Git 父链和字节拒绝夹带与并发改写', async t => {
    ...
    const made = makeReceipt({ requestId: 'a'.repeat(64), operation: 'YANK', pr: structuredClone(pr), current: source,
        ...
        state: { raw: file => file === 'generated/current.json' ? baseline : null }, releases: [], reviewContext: {} });
    ...
    let current = source, lostResponse = true, extraTreeFile = false;
    const mutations = [];
    const call = (endpoint, options = {}) => {
    ...
    const result = appendReviewCommit(made.value, pointer, call);
    ...
    appendReviewCommit(made.value, pointer, call);
