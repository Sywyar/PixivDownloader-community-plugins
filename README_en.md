...
- For a transfer, the recipient chooses to request plugin ownership and enters the current owner's `publisherId/pluginId`, for example `sywyar/douyin`. The wizard verifies current ownership before asking for the recipient's identity and key; the `publisherId` field at that point identifies the recipient's community publisher. After the request is reviewed and merged into `master`, the current owner chooses “I am the current owner: confirm an ownership request” and selects the request. Recipients can use “I am the recipient: confirm an existing request” if their confirmation is still needed. When no request is available, the wizard explains why unmerged, confirmed and stale requests are omitted. The owner's handoff instructions provide the full identifier to copy. A regular transfer takes effect after both parties confirm and human review completes. Never exchange private keys. Recovery evidence and separate review are required when the current owner cannot approve.

For YANK, UNYANK and REVOKE signed with the current personal maintainer's active key, and routine personal key rotation proved by both keys, a protected workflow verifies the request, appends results to the original PR, then merges after checks pass and updates community state. These operations require neither a rebuild nor human approval. Native review objections still block admission. Version publication, ownership transfers, missing key proofs, recovery rotation after key loss or compromise, organization authority and restoration involving independent community restrictions require human review.
