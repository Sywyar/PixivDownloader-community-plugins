...
- For a transfer, the recipient chooses to request plugin ownership using their own account and key. `publisherId` identifies the recipient's community publisher. After the request is reviewed and recorded, the current owner selects it here to confirm; owners can also view handoff instructions first. A regular transfer takes effect after both parties confirm and human review completes. Never exchange private keys. Recovery evidence and separate review are required when the current owner cannot approve.
